    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query) {
                $edit = "<a href='".route('admin.why-choose-us.edit', $query->id)."' class='btn btn-primary'><i class='far fa-edit'></i></a>";
                $delete = "<a href='".route('admin.why-choose-us.destroy', $query->id)."' class='btn btn-danger delete-item' ml-4><i class='fas fa-trash-alt'></i></a>";
                return $edit . " " . $delete;
            })
            ->addColumn('icon', function($query) {
                return '<i style="font-size: 30px;" class="fa fa-'. $query->icon .'"></i>';
